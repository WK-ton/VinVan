...
function CarSaiTai() {
    ...
    useEffect(() => {
        axios.get('http://localhost:8081/carSaiTai/getCars/cars_SatTai')
        .then(res => {
    ...
    const handleDelete = (id) => {
      axios.delete('http://localhost:8081/carSaiTai/delete/cars_SatTai/'+id)
      .then(res => {
        if(res.data.Status === "Success") {
  ...
  return (
    ...
    <Nav className='container' variant="pills" defaultActiveKey="/home">
      ...
      <Nav className='active'>
        <Nav.Link className="font" href='/create/cars_saitai'>+ CARS</Nav.Link>
      </Nav>
      <Nav>
      </Nav>
    </Nav>
                ...
                <tbody>
                    {data&&data.map((cars,index) => { 
                      return <tr key={index}>
                            ...
                            />}</td>
                            <td>
                                <Link to={`/carsEdit_SaiTai/`+ cars.id} className='btn btn-primary btn-sm me-2'>edit</Link>
