...
function CarBangKhen() {
  ...
  return (
    ...
    <Nav className='container' variant="pills" defaultActiveKey="/home">
      ...
      <Nav className='active'>
        <Nav.Link className="font" href='/create/cars_bangkhen'>+ CARS</Nav.Link>
      </Nav>
      <Nav>
      </Nav>
    </Nav>
                ...
                <tbody>
                    {data&&data.map((cars,index) => { 
                      return <tr key={index}>
                            ...
                            />}</td>
                            <td>
                                <Link to={`/carsEdit_BangKhen/`+ cars.id} className='btn btn-primary btn-sm me-2'>edit</Link>
