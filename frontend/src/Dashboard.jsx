...
function Dashboard() {
    ...
    useEffect(() => {
        axios.get('http://localhost:8081/car/getCars')
        .then(res => {
    ...
    const handleDelete = (id) => {
      axios.delete('http://localhost:8081/car/delete/'+id)
      .then(res => {
        if(res.data.Status === "Success") {
                ...
                <tbody>
                    {data&&data.map((cars,index) => { 
                      return <tr key={index}>
                            ...
                            <td>{cars.number}</td>
                            <td>{cars.road}</td>
                            <td>{<img src={`http://localhost:8081/images/`+ cars.image} alt="" className='cars_images' />}</td>
                            <td>
